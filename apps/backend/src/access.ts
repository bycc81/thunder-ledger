...
export async function auth(request: Req, reply: FastifyReply): Promise<boolean> {
  try {
    ...
    else if (c.sub === (process.env.ADMIN_USERNAME ?? 'admin') && c.role === 'admin') { const name = process.env.ADMIN_USERNAME ?? 'admin'; const hash = process.env.ADMIN_PASSWORD_HASH; if (!hash || hash.includes('replace-with-a-bcrypt-hash')) throw new Error('admin password is not configured'); let row = (await getPool().query('SELECT id,status FROM users WHERE username=$1', [name])).rows[0] as { id: string; status: string } | undefined; if (!row) { row = (await getPool().query('INSERT INTO users(id,username,password_hash) VALUES($1,$2,$3) RETURNING id,status', [randomUUID(), name, hash])).rows[0] as { id: string; status: string }; } if (row.status !== 'active') throw new Error('inactive admin'); request.access = { id: row.id, username: name, superAdmin: true }; }
    else request.access = { id: c.sub, username: c.username ?? c.sub, superAdmin: false };
    return true;
  }
...
function canReadBatch(context: Awaited<ReturnType<typeof batchContext>>) { return Boolean(context?.batchRole); }
function canEditBatch(context: Awaited<ReturnType<typeof batchContext>>) { return Boolean(context && (context.batchRole === 'owner' || context.batchRole === 'editor')); }
...
export async function registerAccessRoutes(app: FastifyInstance): Promise<void> {
  app.post<{ Body: { token?: string; username?: string; password?: string } }>('/api/auth/register', async (request, reply) => {
    ...
    if (!token || !username || !password || username.length > 120 || password.length < 8) return reply.code(400).send({ code: 'INVALID_REGISTRATION', message: '注册信息无效' });
    ...
    if (invite.email_or_username !== username) return reply.code(400).send({ code: 'INVITATION_TARGET_MISMATCH', message: '邀请对象不匹配' });
    const client = await pool.connect(); const id = randomUUID();
    try { await client.query('BEGIN'); await client.query('INSERT INTO users(id,username,password_hash) VALUES($1,$2,$3)', [id, username, await bcrypt.hash(password, 12)]); const personalId = randomUUID(); await client.query('INSERT INTO workspaces(id,name,kind,created_by) VALUES($1,$2,$3,$4)', [personalId, `${username} 的个人空间`, 'personal', id]); await client.query('INSERT INTO workspace_members(workspace_id,user_id,role) VALUES($1,$2,$3)', [personalId, id, 'owner']); await client.query('INSERT INTO workspace_members(workspace_id,user_id,role) VALUES($1,$2,$3)', [invite.workspace_id, id, invite.role]); await client.query('UPDATE invitations SET consumed_at=now() WHERE id=$1'); await client.query('COMMIT'); await audit(invite.workspace_id, id, 'register', 'invitation', invite.id); return { userId: id, personalWorkspaceId: personalId }; }
  ...
  app.post<{ Body: { username?: string; password?: string; captchaId?: string; captchaCode?: string } }>('/api/auth/db-login', async (request, reply) => {
    const { username, password } = request.body ?? {}; const row = (await getPool().query('SELECT id,username,password_hash,status FROM users WHERE username=$1', [username ?? ''])).rows[0];
