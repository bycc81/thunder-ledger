...
async function logout() {
  try { await showConfirmDialog({ title: '退出登录', message: '退出后需要重新登录才能继续操作。', confirmButtonText: '退出', confirmButtonColor: '#b42318' }); }
  catch { return; }
  auth.logout();
  store.clear();
...
<template>
  <MobileShell page="profile" :workspace-name="store.selectedWorkspace?.name" @open-workspace="showWorkspace = true" @navigate="navigate">
    <section class="profile-page" data-ai-id="profile-page">
      ...
      <section class="profile-section" data-ai-id="profile-actions"><h2>账户操作</h2><van-button block class="logout-button" plain type="danger" data-ai-id="profile-logout" @click="logout">退出登录</van-button></section>
    </section>
  </MobileShell>
