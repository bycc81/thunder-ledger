...
const filteredBatches = computed(() => store.batches.filter((batch) => {
  ...
  return !keyword || batch.name.toLowerCase().includes(keyword);
}));
...
<template>
  ...
  <DangerConfirmDialog v-model:show="showDeleteConfirm" title="删除批次" :message="pendingDelete ? `确定删除“${pendingDelete.name}”吗？删除后，该批次将不再显示在当前工作区；已有记录会保留，无法继续操作。` : ''" confirm-text="删除" ai-id="batch-delete-confirm" :loading="Boolean(deletingId)" @confirm="deleteBatch" />
</template>
...
.filters :deep(.van-search) { width:100%; padding:0; }
