...
function externalSourceUrl(purchase: InventoryDetail['purchases'][number]) {
  const source = purchase.sourceUrl?.trim();
  if (!source || (/^[a-z][a-z\d+.-]*:/i.test(source) && !/^https?:\/\//i.test(source))) return '';
  try {
    const url = new URL(/^https?:\/\//i.test(source) ? source : `https://${source}`);
    return ['http:', 'https:'].includes(url.protocol) && url.hostname ? url.href : '';
  } catch { return ''; }
}
function hasSourceUrl(purchase: InventoryDetail['purchases'][number]) { return Boolean(externalSourceUrl(purchase)); }
...
<template>
  <div class="inventory-page" :data-ai-id="productId ? 'inventory-detail-page' : 'inventory-list-page'">
    ...
    <main v-else-if="!productId" class="content">
      <header class="page-heading" data-ai-id="inventory-list-header"><div><span class="eyebrow">当前批次</span><h1>库存</h1><p>同一种商品合并在一起卖</p></div><van-button v-if="canEdit" class="tl-button--secondary" type="primary" data-ai-id="inventory-purchase-create" @click="createPurchase">新增采购</van-button></header>
    ...
    <main v-else-if="detail" class="content">
      ...
      <section data-ai-id="inventory-purchase-list"><h2>采购记录</h2><article v-for="purchase in detail.purchases" :key="purchase.id" class="purchase-item" :data-ai-id="`inventory-purchase-item-${purchase.id}`"><div class="purchase-head"><div class="purchase-main"><strong>{{ purchase.channelName }}</strong><p class="purchase-meta">{{ formatDateTime(purchase.occurredAt) }} · {{ purchase.payerUsername }} 付款</p><small class="purchase-cost">成本由：{{ shares(purchase) }}</small><p v-if="purchase.note" class="purchase-note">备注：{{ purchase.note }}</p></div><div class="purchase-side"><b>{{ purchase.quantity }} 件 · {{ money(purchase.totalCost) }}</b><div v-if="hasSourceUrl(purchase) || canEdit" class="purchase-actions"><a v-if="hasSourceUrl(purchase)" :href="externalSourceUrl(purchase)" target="_blank" rel="noreferrer" :data-ai-id="`inventory-purchase-source-${purchase.id}`">查看来源</a><button v-if="canEdit" type="button" class="purchase-edit" :data-ai-id="`inventory-purchase-edit-${purchase.id}`" @click="editPurchase(purchase)">编辑</button></div></div></div></article></section>
