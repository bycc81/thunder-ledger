...
const showProductPicker = ref(false); const showSellerPicker = ref(false);
...
const productOptions = computed(() => products.value.map((product) => ({ text: product.productName, value: product.productId })));
...
function chooseProduct({ selectedOptions }: { selectedOptions: Array<{ value?: string }> }) { productId.value = selectedOptions[0]?.value ?? ''; showProductPicker.value = false; }
