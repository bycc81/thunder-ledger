...
新模块/新页面必须使用 `docs/changes/YYYYMMDD-feature/` 目录，并包含 `product-brief.md`、`ux-spec.md`、`prototype/index.html`、`design.md`、`test-plan.md` 和 `changelog.md`。原型只使用原生 HTML/CSS/少量 JavaScript，可直接浏览器打开，不接真实 API。

已有页面的小范围样式、文案或字段调整不触发完整 Product/UX 流程。若已有页面优化明显改变布局、信息层级、主流程或操作路径，先询问用户是否需要原型；用户要求时才进入该阶段。截图仅用于留档、远程审查或视觉回归，不作为默认门禁。不默认读取 BMAD、OpenSpec 或其他外部项目全文。
