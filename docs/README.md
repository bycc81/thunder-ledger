...
阅读入 定义；页面/API 契约是 `standards/change-templates/design.md` 的章节。`data-ai-id` 通用规范见 `standards/frontend/data-ai-id-guidelines.md`，具体前端需求在 `changes/` 下保留 ID 注册表实例。
...
口建议：先看 `product-requirements.md`，再看 `architecture/`、`roadmap/` 和 `deployment.md`；开发任务遵循 `standards/`，每个批次的实际记录放在 `changes/`。安全规范统一见 `standards/security.md`。
