...
开发前端可单独运行在 `http://localhost:5174`，本地后端运行在 `http://localhost:3000`。仓库根目录 `.env` 中的 `DATABASE_URL` 用于本地后端连接 `127.0.0.1:5432`，`DATABASE_URL_DOCKER` 仅供 Compose 内的 API 和迁移容器使用。`apps/backend` 的 `npm run dev` 会自动读取根目录 `.env`。
